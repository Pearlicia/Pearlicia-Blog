...
def lambda_handler(event, context):
    ...
    try:
        ...
        log_group_name = unquote(deeplink).split("/log-group/")[1].split("/log-events")[0]
        log_stream_name = deeplink.split("/log-events/")[1]

        log_events = client.get_log_events(logGroupName=log_group_name, logStreamName=log_stream_name)

        for event in log_events['events']:
            message = event['message']
            if "ERROR:" in message or "Failed tests:" in message:
    ...
    data = {
        "blocks": [
            ...
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"*Pipeline name:* {pipeline_name}\n*Status:* {status}\n*Source:* {source}"
                },
                "accessory": {
                    "type": "image",
                    "image_url": gif_url,
...
log_group_name_decoded = 'your_log_group_name'
...
try:
    while True:
        ...
        for event in log_events['events']:
            ...
            error_occurred_match = re.search(r"Error occurred: (.+)", message)
            if error_occurred_match:
                error_message = error_occurred_match.group(1)
                print(f"Error occurred: {error_message}")
                results_message.append(f"Error occurred: {error_message}\n")
